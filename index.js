...
const API_KEY = "live_gOEGn6u8bLSog0dN63rN2dx6pU5EkEQ15wifWJF56eXpSmqjbJJ3kiaAPbDUhkQo";
...
async function axiosHandleBreedSelect(){
  ...
  const response = await axios.get(`https://api.thecatapi.com/v1/images/search?limit=10&breed_ids=${breedSelect.value}`, {
  ...
  infoDump.textContent = obj.breeds[0]
.description;
/**
  ...
  axios.get("https://api.thecatapi.com/v1/favourites").then((res) => {
    ...
    res.data.forEach((item) => {
      ...
      if (item.image_id === imgId) {
        ...
        // delete
        deleted = true;
        axios.delete(`https://api.thecatapi.com/v1/favourites/${item.id}`).then(res => console.log(res))
      }
    });
...
function getFavourites(){
  ...
  .then(response => {
    console.log('ALL FAVS:::',response.data);
    Carousel.clear()
 ...
 * An empty array and this error that i cant figure out: index.js:101 Uncaught (in promise) TypeError: Cannot read properties of undefined (reading 'breeds')
    at HTMLSelectElement.handleBreedSelect (index.js:101:35)
